...
// define NO_XSFR_DEFINE before include CH559.H to disable xSFR define xdata
// address by _at_
...
#define bCODE_WE                                                               \
    0x08 // enable flash-ROM (include code & data area) being program or
         // erasing: 0=writing protect, 1=enable program and erase
#define bDATA_WE                                                               \
...
__sfr
    __at(0xB2) PLL_CFG; // PLL clock config: lower 5 bits for PLL clock Fpll,
                        // upper 3 bits for USB 4x clock Fusb4x, Write@SafeMode
...
__sfr __at(0xB3) CLOCK_CFG; // system clock config: lower 5 bits for system
                            // clock Fsys, Write@SafeMode
#define bOSC_EN_INT                                                            \
...
__sfr __at(0xFF)
    WDOG_COUNT; // watch-dog count, count by clock frequency Fsys/262144

/*  Interrupt Registers  */
__sfr __at(0xA8) IE; // interrupt enable
__sbit __at(0xA8 +
            7) EA; // enable global interrupts: 0=disable, 1=enable if E_DIS=0
__sbit __at(0xA8 + 6)
    E_DIS; // disable global interrupts, intend to inhibit interrupt during some
...
#define ROM_CMD_PROG                                                           \
    0x9A // WriteOnly: flash-ROM word program operation command, for changing
         // some ROM bit of a word from 1 to 0
#define ROM_CMD_ERASE                                                          \
    0xA6 // WriteOnly: flash-ROM sector erase operation command, for changing
         // all ROM bit of 1KBytes from 0 to 1
...
#define bROM_CMD_ERR                                                           \
    0x02 // ReadOnly: flash-ROM operation command error flag: 0=command
         // accepted, 1=unknown command
#define bROM_CMD_TOUT                                                          \
    0x01 // ReadOnly: flash-ROM operation result: 0=success, 1=operation time
         // out
...
__sbit __at(0x90 + 6) MISO; // master serial data input or slave serial data
                            // output for SPI0, not 5VT
__sbit __at(0x90 + 5) MOSI; // master serial data output or slave serial data
                            // input for SPI0, not 5VT
...
__sbit
    __at(0x90 +
         1) T2EX; // external trigger input for timer2 reload & capture, not 5VT
...
__sbit __at(0xB0 + 4)
    XCS0; // xBUS chip-selection 0# output, for address range 0x4000~0x7FFF
...
#define bPWM1_PIN_X                                                            \
    0x80 // PWM1/PWM2 alternate pin enable: 0=PWM1/PWM2 on P2.4/P2.5,
         // 1=PWM1/PWM2 on P4.3/P4.5
#define bTMR3_PIN_X                                                            \
...
#define bUART0_PIN_X                                                           \
    0x10 // UART0 alternate pin enable: 0=RXD0/TXD0 on P3.0/P3.1, 1=RXD0/TXD0 on
         // P0.2/P0.3
#define bXBUS_EN                                                               \
    0x08 // xBUS function enable: 0=disable, 1=enable port 0 used as data bus
         // and P3.6/P3.7 used as bus writing/reading strobe during accessing
         // xBUS
#define bXBUS_CS_OE 0x04 // xBUS chip-selection output enable: 0=disable output,
//   1=output CS0 (chip-selection 0#, low action) at P3.4, output inversion of
//   address_15 (be equal to CS1, low action) at P3.3 if ALE disabled
#define bXBUS_AH_OE                                                            \
...
#define bXBUS_AL_OE                                                            \
    0x01 // xBUS low address output enable: 0=multiplex low address and data bus
         // during accessing xBUS, 1=output address_0~7 at P4.0~P4.5 & P3.5 &
         // P2.7 directly
...
__sbit __at(0x88 + 7) TF1; // timer1 overflow & interrupt flag, auto cleared
                           // when MCU enter interrupt routine
__sbit __at(0x88 + 6) TR1; // timer1 run enable
__sbit __at(0x88 + 5) TF0; // timer0 overflow & interrupt flag, auto cleared
                           // when MCU enter interrupt routine
__sbit __at(0x88 + 4) TR0; // timer0 run enable
__sbit __at(0x88 + 3)
    IE1; // INT1 interrupt flag, auto cleared when MCU enter interrupt routine
__sbit __at(0x88 + 2)
    IT1; // INT1 interrupt type: 0=low level action, 1=falling edge action
__sbit __at(0x88 + 1)
    IE0; // INT0 interrupt flag, auto cleared when MCU enter interrupt routine
__sbit __at(0x88 + 0)
    IT0; // INT0 interrupt type: 0=low level action, 1=falling edge action
__sfr __at(0x89) TMOD; // timer 0/1 mode
#define bT1_GATE                                                               \
    0x80 // gate control of timer1: 0=timer1 run enable while TR1=1, 1=timer1
         // run enable while P3.3 (INT1) pin is high and TR1=1
#define bT1_CT                                                                 \
...
//   timer1
#define bT0_GATE                                                               \
    0x08 // gate control of timer0: 0=timer0 run enable while TR0=1, 1=timer0
         // run enable while P3.2 (INT0) pin is high and TR0=1
#define bT0_CT                                                                 \
...
__sbit __at(0x98 + 7)
    SM0; // UART0 mode bit0, selection data bit: 0=8 bits data, 1=9 bits data
__sbit __at(0x98 +
            6) SM1; // UART0 mode bit1, selection baud rate: 0=fixed, 1=variable
// SM0 & SM1: UART0 mode
...
__sbit __at(0x98 + 2)
    RB8; // 9th data bit received in mode 2/3, or stop bit received for mode 1
__sbit __at(0x98 +
            1) TI; // transmit interrupt flag, set by hardware after completion
                   // of a serial transmittal, need software clear
__sbit __at(0x98 +
            0) RI; // receive interrupt flag, set by hardware after completion
                   // of a serial receiving, need software clear
__sfr __at(0x99)
    SBUF; // UART0 data buffer: reading for receiving, writing for transmittal

/*  Timer2/Capture2 Registers  */
__sfr __at(0xC8) T2CON; // timer 2 control
__sbit __at(0xC8 +
            7) TF2; // timer2 overflow & interrupt flag, need software clear,
                    // the flag will not be set when either RCLK=1 or TCLK=1
__sbit __at(0xC8 + 7) CAP1F; // timer2 capture 1 interrupt flag, set by T2 edge
                             // trigger if bT2_CAP1_EN=1, need software clear
__sbit __at(0xC8 + 6) EXF2; // timer2 external flag, set by T2EX edge trigger if
                            // EXEN2=1, need software clear
__sbit __at(0xC8 + 5) RCLK; // selection UART0 receiving clock: 0=timer1
                            // overflow pulse, 1=timer2 overflow pulse
__sbit __at(0xC8 + 4) TCLK; // selection UART0 transmittal clock: 0=timer1
                            // overflow pulse, 1=timer2 overflow pulse
__sbit __at(0xC8 + 3) EXEN2; // enable T2EX trigger function: 0=ignore T2EX,
                             // 1=trigger reload or capture by T2EX edge
__sbit __at(0xC8 + 2) TR2;   // timer2 run enable
__sbit __at(0xC8 +
            1) C_T2; // timer2 clock source selection: 0=timer base internal
                     // clock, 1=external edge counter base T2 falling edge
__sbit __at(0xC8 + 0) CP_RL2; // timer2 function selection (force 0 if RCLK=1 or
                              // TCLK=1): 0=timer and auto reload if count
                              // overflow or T2EX edge, 1=capture by T2EX edge
...
#define bT1_CLK                                                                \
    0x20 // timer1 internal clock frequency selection: 0=standard clock,
         // Fsys/12, 1=faster clock, Fsys/4 if bTMR_CLK=0 or Fsys if bTMR_CLK=1
#define bT0_CLK                                                                \
    0x10 // timer0 internal clock frequency selection: 0=standard clock,
         // Fsys/12, 1=faster clock, Fsys/4 if bTMR_CLK=0 or Fsys if bTMR_CLK=1
...
#define T2OE                                                                   \
    0x02 // enable timer2 generated clock output: 0=disable output, 1=enable
         // clock output at T2 pin, frequency = TF2/2
#define bT2_CAP1_EN                                                            \
    0x01 // enable T2 trigger function for capture 1 of timer2 if RCLK=0 &
         // TCLK=0 & CP_RL2=1 & C_T2=0 & T2OE=0
...
#define bT3_IE_END                                                             \
    0x80 // enable interrupt for capture mode count timeout (exceed end value)
         // or PWM mode cycle end
...
#define bT3_EN_CK_SE                                                           \
    0x01 // enable to accessing divisor setting register, else enable to
         // accessing current count register
...
__sfr __at(
    0xAB) T3_DMA_CN; // DMA remainder word count, automatic decreasing after DMA
// _sfr16 __at (0xAC) T3_DMA;// DMA address, must even address, little-endian,
// automatic increasing after DMA
__sfr __at(
    0xAC) T3_DMA_AL; // DMA address low byte, automatic increasing after DMA
__sfr __at(
    0xAD) T3_DMA_AH; // DMA address high byte, automatic increasing after DMA
...
#define bPWM2_POLAR                                                            \
    0x40 // PWM2 output polarity if bPWM_MOD_MFM=0: 0=default low and high
         // action, 1=default high and low action
#define bMFM_BUF_EMPTY                                                         \
    0x40 // ReadOnly: MFM empty buffer status if bPWM_MOD_MFM=1
#define bPWM_POLAR                                                             \
    0x20 // PWM output polarity: 0=default low and high action, 1=default high
         // and low action
#define bPWM_IF_END                                                            \
...
__sbit __at(0xF8 + 7)
    S0_FST_ACT; // ReadOnly: indicate first byte received status for SPI0
__sbit __at(0xF8 + 6) S0_IF_OV; // interrupt flag for slave mode FIFO overflow,
                                // direct bit address clear or write 1 to clear
__sbit __at(0xF8 +
            5) S0_IF_FIRST; // interrupt flag for first byte received, direct
                            // bit address clear or write 1 to clear
__sbit __at(0xF8 +
            4) S0_IF_BYTE; // interrupt flag for a byte data exchanged, direct
                           // bit address clear or write 1 to clear or accessing
                           // FIFO to clear if bS0_AUTO_IF=1
...
__sfr __at(0xF9)
    SPI0_DATA; // FIFO data port: reading for receiving, writing for transmittal
...
__sfr __at(0xB5)
    SPI1_DATA; // data port: reading for receiving, writing for transmittal
...
__sfr __at(0x9A) SER1_FIFO; // UART1 FIFO data port: reading for receiving,
                            // writing for transmittal
...
__sfr __at(0x97) SER1_ADDR; // UART1 slave address for multi-device
                            // communication, value 0xFF is disable
...
/*  ADC Registers  */
// _sfr16 __at (0xEC) ADC_DMA;// DMA address, must even address, little-endian,
// automatic increasing after DMA
__sfr __at(0xEC)
    ADC_DMA_AL; // DMA address low byte, automatic increasing after DMA
__sfr __at(0xED)
    ADC_DMA_AH; // DMA address high byte, automatic increasing after DMA
__sfr __at(0xEE)
    ADC_DMA_CN; // DMA remainder word count, automatic decreasing after DMA
...
#define bADC_CHANN_ID                                                          \
    0x04 // ReadOnly: current channel ID for channel automatic switch mode:
         // 0=AIN0 or AIN6, 1=AIN1 or AIN4 or AIN7
#define bADC_DATA_OK                                                           \
...
#define MASK_ADC_CYCLE                                                         \
    0x0F // bit mask of ADC cycle (ADC clock number): 0=manual sample,
...
#define bADC_AIN7_CHK_EN                                                       \
    0x01 // control AIN7 level check module power: 0=shut down, 1=enable power
         // for AIN7 level check
__sfr __at(0xF7) ADC_EX_SW; // ADC extend switch control
#define bADC_SW_AIN7_H                                                         \
    0x80 // internal AIN7 extend switch control: 0=float AIN7, 1=tie AIN7 to
         // high level VDD33
#define bADC_SW_AIN6_L                                                         \
    0x40 // internal AIN6 extend switch control: 0=float AIN6, 1=tie AIN6 to low
         // level GND
#define bADC_SW_AIN5_H                                                         \
    0x20 // internal AIN5 extend switch control: 0=float AIN5, 1=tie AIN5 to
         // high level VDD33
...
#define bUEP_R_TOG                                                             \
    0x80 // expected data toggle flag of USB endpoint X receiving (OUT):
         // 0=DATA0, 1=DATA1
#define bUEP_T_TOG                                                             \
    0x40 // prepared data toggle flag of USB endpoint X transmittal (IN):
         // 0=DATA0, 1=DATA1
...
#define MASK_UEP_R_RES                                                         \
    0x0C // bit mask of handshake response type for USB endpoint X receiving
         // (OUT)
...
__sbit
    __at(0xD8 +
         7) U_IS_NAK; // ReadOnly: indicate current USB transfer is NAK received
__sbit __at(0xD8 +
            6) U_TOG_OK; // ReadOnly: indicate current USB transfer toggle is OK
__sbit __at(0xD8 + 5) U_SIE_FREE; // ReadOnly: indicate USB SIE free status
__sbit __at(0xD8 +
            4) UIF_FIFO_OV; // FIFO overflow interrupt flag for USB, direct bit
                            // address clear or write 1 to clear
__sbit __at(0xD8 +
            3) UIF_HST_SOF; // host SOF timer interrupt flag for USB host,
                            // direct bit address clear or write 1 to clear
__sbit __at(0xD8 +
            2) UIF_SUSPEND; // USB suspend or resume event interrupt flag,
                            // direct bit address clear or write 1 to clear
__sbit __at(0xD8 +
            1) UIF_TRANSFER; // USB transfer completion interrupt flag, direct
                             // bit address clear or write 1 to clear
__sbit __at(0xD8 +
            0) UIF_DETECT; // device detected event interrupt flag for USB host
                           // mode, direct bit address clear or write 1 to clear
__sbit __at(0xD8 + 0)
    UIF_BUS_RST; // bus reset event interrupt flag for USB device mode, direct
                 // bit address clear or write 1 to clear
__sfr __at(0xD9) USB_INT_ST; // ReadOnly: USB interrupt status
#define bUIS_IS_NAK                                                            \
    0x80 // ReadOnly: indicate current USB transfer is NAK received for USB
         // device mode
...
#define MASK_UIS_ENDP                                                          \
    0x0F // ReadOnly: bit mask of current transfer endpoint number for USB
         // device mode
#define MASK_UIS_H_RES                                                         \
    0x0F // ReadOnly: bit mask of current transfer handshake response for USB
...
__sfr __at(0xE3)
    USB_DEV_AD; // USB device address, lower 7 bits for USB device address
...
#define bUH_DP_PD_DIS                                                          \
    0x20 // disable USB DP or HP pulldown resistance: 0=enable pulldown,
         // 1=disable
#define bUH_DM_PD_DIS                                                          \
    0x10 // disable USB DM or HM pulldown resistance: 0=enable pulldown,
         // 1=disable
#define bUH_DIFF_IN                                                            \
...
#ifndef NO_XSFR_DEFINE
#define UEP0_DMA_H                                                             \
    (*(UINT8XV *)0x2448); // endpoint 0&4 buffer start address high byte
#define UEP0_DMA_L                                                             \
    (*(UINT8XV *)0x2449); // endpoint 0&4 buffer start address low byte
...
#ifndef NO_XSFR_DEFINE
#define UEP1_DMA_H                                                             \
    (*(UINT8XV *)0x244A); // endpoint 1 buffer start address high byte
#define UEP1_DMA_L                                                             \
    (*(UINT8XV *)0x244B); // endpoint 1 buffer start address low byte
...
#ifndef NO_XSFR_DEFINE
#define UEP2_DMA_H                                                             \
    (*(UINT8XV *)0x244C); // endpoint 2 buffer start address high byte
#define UEP2_DMA_L                                                             \
    (*(UINT8XV *)0x244D); // endpoint 2 buffer start address low byte
...
#ifndef NO_XSFR_DEFINE
#define UEP3_DMA_H                                                             \
    (*(UINT8XV *)0x244E); // endpoint 3 buffer start address high byte
#define UEP3_DMA_L                                                             \
    (*(UINT8XV *)0x244F); // endpoint 3 buffer start address low byte
...
// UINT8XV  UH_RX_DMA_H _at_ 0x244C;   // host rx endpoint buffer start address
// high byte
#define UH_RX_DMA_H UEP2_DMA_H
#define pUH_RX_DMA_H pUEP2_DMA_H
// UINT8XV  UH_RX_DMA_L _at_ 0x244D;   // host rx endpoint buffer start address
// low byte
...
// UINT8XV  UH_TX_DMA_H _at_ 0x244E;   // host rx endpoint buffer start address
// high byte
#define UH_TX_DMA_H UEP3_DMA_H
#define pUH_TX_DMA_H pUEP3_DMA_H
// UINT8XV  UH_TX_DMA_L _at_ 0x244F;   // host rx endpoint buffer start address
// low byte
...
#define LED_DMA_AH                                                             \
    (*(UINT8XV                                                                 \
           *)0x2884); // DMA address high byte, automatic increasing after DMA
#define LED_DMA_AL                                                             \
    (*(UINT8XV                                                                 \
           *)0x2885); // DMA address low byte, automatic increasing after DMA
...
#define LED_DMA_CN                                                             \
    (*(UINT8XV                                                                 \
           *)0x2886); // DMA remainder word count, just main buffer and exclude
                      // aux buffer, automatic decreasing after DMA
...
#ifndef NO_XSFR_DEFINE
#define LED_DMA_XH                                                             \
    (*(UINT8XV *)0x2888); // aux buffer DMA address high byte, automatic
                          // increasing after DMA
#define LED_DMA_XL                                                             \
    (*(UINT8XV *)0x2889); // aux buffer DMA address low byte, automatic
                          // increasing after DMA
