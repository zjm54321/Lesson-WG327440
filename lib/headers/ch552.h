...
#define bCODE_WE                                                               \
    0x08 // enable flash-ROM (include code & Data-Flash) being program or
...
__sfr __at(0xB9) CLOCK_CFG; // system clock config: lower 3 bits for system
...
__sfr __at(0xFF)
    WDOG_COUNT; // watch-dog count, count by clock frequency Fsys/65536
...
__sfr __at(0x8E) ROM_DATA_L; // data low byte for flash-ROM writing, data byte
                             // for Data-Flash reading/writing
...
__sbit __at(0x90 + 6)
    MISO; // master serial data input or slave serial data output for SPI0
__sbit __at(0x90 + 6) RXD1; // RXD input for UART1
__sbit __at(0x90 + 6) TIN4; // TIN4 for Touch-Key
__sbit __at(0x90 + 5)
    MOSI; // master serial data output or slave serial data input for SPI0
...
__sbit __at(0x90 +
            1) T2EX; // external trigger input for timer2 reload & capture
...
__sfr __at(0x93)
    P1_DIR_PU; // port 1 direction for push-pull or pullup enable for open-drain
...
__sfr __at(0x97)
    P3_DIR_PU; // port 3 direction for push-pull or pullup enable for open-drain
...
__sbit __at(0x98 + 6) SM1; // UART0 mode bit1, selection baud rate: 0=fixed,
                           // 1=variable SM0 & SM1: UART0 mode
...
__sbit __at(0xC0 + 7)
    U1SM0; // UART1 mode, selection data bit: 0=8 bits data, 1=9 bits data
__sbit __at(0xC0 + 5)
    U1SMOD; // UART1 2X baud rate selection: 0=slow(Fsys/32/(256-SBAUD1)),
...
__sbit __at(0xC0 + 2) U1RB8; // 9th data bit received in 9 bits data mode, or
                             // stop bit received for 8 bits data mode
__sbit __at(0xC0 +
            1) U1TI; // transmit interrupt flag, set by hardware after
                     // completion of a serial transmittal, need software clear
__sbit __at(0xC0 +
            0) U1RI; // receive interrupt flag, set by hardware after completion
                     // of a serial receiving, need software clear
__sfr __at(0xC1)
    SBUF1; // UART1 data buffer: reading for receiving, writing for transmittal
...
__sbit __at(0x80 + 6)
    CMP_IF; // flag for comparator result changed, direct bit address clear
__sbit
    __at(0x80 +
         5) ADC_IF; // interrupt flag for ADC finished, direct bit address clear
__sbit __at(0x80 +
            4) ADC_START; // set 1 to start ADC, auto cleared when ADC finished
__sbit
    __at(0x80 +
         3) CMP_CHAN; // comparator IN- input channel selection: 0=AIN1, 1=AIN3
__sbit __at(0x80 +
            1) ADC_CHAN1; // ADC/comparator IN+ channel selection high bit
...
#define bCMP_EN                                                                \
    0x04 // control comparator power: 0=shut down comparator, 1=enable power for
         // comparator
#define bADC_CLK                                                               \
    0x01 // ADC clock frequency selection: 0=slow clock, 384 Fosc cycles for
         // each ADC, 1=fast clock, 96 Fosc cycles for each ADC
...
__sbit __at(0xD8 +
            1) UIF_TRANSFER; // USB transfer completion interrupt flag, direct
                             // bit address clear or write 1 to clear
__sbit __at(0xD8 + 0) UIF_BUS_RST; // bus reset event interrupt flag, direct bit
                                   // address clear or write 1 to clear
__sfr __at(0xD9) USB_INT_ST;       // ReadOnly: USB interrupt status
#define bUIS_IS_NAK                                                            \
    0x80 // ReadOnly: indicate current USB transfer is NAK received
...
#define MASK_UIS_TOKEN                                                         \
    0x30 // ReadOnly: bit mask of current token PID code received
...
#define MASK_UIS_ENDP                                                          \
    0x0F // ReadOnly: bit mask of current transfer endpoint number
...
//   01: enable USB device and disable internal pullup resistance, need external
//   pullup resistance 1x: enable USB device and enable internal pullup
//   resistance
#define bUC_INT_BUSY                                                           \
    0x08 // enable automatic responding busy during interrupt flag UIF_TRANSFER
         // valid
